...
package xdean.OpenGLSuperBible.share.TGA;
...
//	 public static final Order ARGB = new Order(16, 8, 0, 24);
	public static final Order RGBA = new Order(24, 16, 8, 0);
	...
	public static int getBits(byte[] buffer) {
		return (buffer[16] & 0xFF) | (buffer[17] & 0xFF) << 8;
	}
	...
	public static int[] read(byte[] buffer, Order order) throws IOException {
		...
		int descriptor = buffer[17] & 0xFF ^ UPPER_ORIGIN;// It seems this class
															// use the wrong
															// up-down
															// direction. So
															// I convert it.
		...
		case COLORMAP: {
			...
			pixels = createPixelsFromColormap(width, height, colormapDepth,
					buffer, imageDataOffset, buffer, colormapOrigin,
					descriptor, order);
		}
			break;
		case RGB:
			...
			break;
		case GRAYSCALE:
			pixels = createPixelsFromGrayscale(width, height, depth, buffer,
					18, descriptor, order);
			break;
		case COLORMAP_RLE: {
			int imageDataOffset = 18 + (colormapDepth / 8) * colormapLength;
			byte[] decodeBuffer = decodeRLE(width, height, depth, buffer,
					imageDataOffset);
			pixels = createPixelsFromColormap(width, height, colormapDepth,
					decodeBuffer, 0, buffer, colormapOrigin, descriptor, order);
		}
			break;
		case RGB_RLE: {
		...
		case GRAYSCALE_RLE: {
			byte[] decodeBuffer = decodeRLE(width, height, depth, buffer, 18);
			pixels = createPixelsFromGrayscale(width, height, depth,
					decodeBuffer, 0, descriptor, order);
		}
			break;
		default:
	...
	private static byte[] decodeRLE(int width, int height, int depth,
	...
	private static int[] createPixelsFromColormap(int width, int height,
			int depth, byte[] bytes, int offset, byte[] palette,
			int colormapOrigin, int descriptor, Order order) throws IOException {
		int[] pixels = null;
	...
	private static int[] createPixelsFromRGB(int width, int height, int depth,
			byte[] bytes, int offset, int descriptor, Order order)
			throws IOException {
		int[] pixels = null;
		int rs = order.redShift;
		int gs = order.greenShift;
	...
	private static int[] createPixelsFromGrayscale(int width, int height,
			int depth, byte[] bytes, int offset, int descriptor, Order order)
			throws IOException {
		int[] pixels = null;
		int rs = order.redShift;
		int gs = order.greenShift;
