...
public class TGAWriter {
	...
	public static byte[] write(int[] pixels, int width, int height,
			TGAReader.Order order) {
		return write(pixels, width, height, order, EncodeType.AUTO);
	}

	public static byte[] write(int[] pixels, int width, int height,
			TGAReader.Order order, EncodeType encodeType) {
	...
	private static int writeRaw(int[] pixels, byte[] buffer, int index,
			int elementCount, TGAReader.Order order) {
		if (elementCount == 3) {
			// BGR
	...
	private static int getEncodeSize(int[] pixels, int width, int elementCount) {
		...
		for (int i = 0; i < pixels.length; i++) {
			if (mode == MODE_RESET) {
				...
				mode = (color == pixels[i]) ? MODE_SAME_COLOR
						: MODE_DIFFERENT_COLOR;
				color = pixels[i];
	...
	private static int encodeRLE(int[] pixels, int width, int elementCount,
			TGAReader.Order order, byte[] buffer, int index) {
		...
		for (int i = 0; i < pixels.length; i++) {
			if (mode == MODE_RESET) {
				...
				if (color != pixels[i]) {
					// packet + rleData
					index = encodeRLE(buffer, index, color, i - start,
							elementCount, order);
					mode = MODE_SELECT;
					color = pixels[i];
					start = i;
				} else if ((i - start) >= 127) {
					index = encodeRLE(buffer, index, color, 128, elementCount,
							order);
					mode = MODE_RESET;
				}
			} else if (mode == MODE_DIFFERENT_COLOR) {
				if (color == pixels[i]) {
					// packet + rawData * count
					index = encodeRLE(buffer, index, pixels, start, i - 1
							- start, elementCount, order);
					mode = MODE_SAME_COLOR;
					color = pixels[i];
					start = i - 1;
				} else if ((i - start) >= 127) {
					index = encodeRLE(buffer, index, pixels, start, 128,
							elementCount, order);
					mode = MODE_RESET;
				}
			}
			...
			if ((i + 1) % width == 0 && mode != MODE_RESET) {
				if (mode == MODE_SAME_COLOR) {
					index = encodeRLE(buffer, index, color, i - start + 1,
							elementCount, order);
				} else {
					// MODE_SELECT or MODE_DIFFERENT_COLOR
					index = encodeRLE(buffer, index, pixels, start, i - start
							+ 1, elementCount, order);
				}
				mode = MODE_RESET;
			}
	...
	private static int encodeRLE(byte[] buffer, int index, int color,
			int count, int elementCount, TGAReader.Order order) {
	...
	private static int encodeRLE(byte[] buffer, int index, int[] pixels,
			int start, int count, int elementCount, TGAReader.Order order) {
	...
	private static final byte[] FOOTER = { 0, 0, 0, 0, 0, 0, 0, 0, 84, 82, 85,
			69, 86, 73, 83, 73, 79, 78, 45, 88, 70, 73, 76, 69, 46, 0 }; // TRUEVISION-XFILE
